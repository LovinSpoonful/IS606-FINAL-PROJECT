...
Alternately, if we reverse the model to Employment vs Imports, the intercept is 17.81 and the slope is -0.0134.  This implies that if the U.S. imported no manufactured goods from China, there would be 17.8 million manufacturing jobs (+- 400,000). It also implies that every billion USD in imports removes 13,400 jobs.  

At face value these results strongly suggest that we can reject the null hypothesis ("Imports and Employment are not related").  They prove temporal precedence and covaration.  But we are not finished yet.  We must now evaluate alternative explanations.  This is accomplished using multivariate linear regression analysis.  


```{r, eval= FALSE, echo= FALSE}

# Example of comparing employment to another variable
lm_alt  <- lm(Employment ~ Balance, data = study)
summary(lm_alt)

```

Before proceeding to a multi-variate model, we examine each variable's individual relationship to Employment:  

```
Individual Linear Models - Relation to Employment

Variable      Slope      Adj R-Squared    p-value
------------  ---------  -------------    -------
Imports       -   13.63     0.9272        0.00001
Exchange       13402.00     0.0922        0.05364
WTO           - 4200.70     0.8312        0.00001
Consumption   -    0.06     0.7397        0.00001
Productivity      51.92   - 0.0260        0.62860
Population    -   81.29     0.8158        0.00001

```
Variables with negligible Slope, low R-Squared values, or high p-values can be excluded from further study, because they are insignificant.  This eliminates Exchange, Consumption and Productivity.  We will perform a multivariate test on Imports, WTO membership and Population.  


```{r}

# perform multi-variate regression test on three variables
lm_all <- lm(Employment ~ Imports + WTO + Population, data = study)
summary(lm_all)

```

We see that Population now has a p-value of 0.61, eliminating it from further consideration.  
...
The slope of the WTO line is -1410, with a p-value of 0.0017.  This indicates with a 99.8% confidence level a relationship between China's entry into the WTO and U.S. manufacturing employment - the impact being 1.5 million jobs lost per year.  Excluding the effects of WTO membership, the relationship of Imports to Employment is 9,800 jobs lost per 1 billion in imports.  
